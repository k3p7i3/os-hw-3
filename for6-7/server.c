...
int visitor;                                 // stores visitor client socket
sem_t visitor_read_sem, visitor_write_sem;
...
void createNotifyString(int client_id, struct EventMessage *message) {
    ...
    if (message->status == WAIT_FOR_REVIEW) {
        sprintf(notify_buffer, "Programmer %d sent his task for review\n", client_id + 1);
    }
...
void handleProgrammerClient(int client_id) {
    ...
    for (;;) {
        ...
        // notify visitor
        sem_wait(&visitor_write_sem);
        createNotifyString(client_id, &message);
        sem_post(&visitor_read_sem);
...
void close_sources() {
    ...
    sem_destroy(&visitor_read_sem);
    sem_destroy(&visitor_write_sem);
    if (visitor > 0) {
        close(visitor);
    }
...
int main(int argc, char *argv[]) {
    ...
    sem_init(&visitor_write_sem, 1, 1);
    sem_init(&visitor_read_sem, 1, 0);
    ...
    visitor = acceptTCPConnection(server_socket);
    for (;;) {
        sem_wait(&visitor_read_sem);
        int len = strlen(notify_buffer);
        send(visitor, notify_buffer, len + 1, 0);
        sem_post(&visitor_write_sem);
    }
